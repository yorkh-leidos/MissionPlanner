...
{
    public partial class SitePlanUI : Form
    {
        ...
        public struct SitePlanData
        {
            ...
            public decimal pictures_per_waypoint;
            public decimal picture_overlap_angle;
        ...
        public SitePlanUI(SitePlanPlugin plugin)
        {
        ...
        private void SitePlanUI_Load(object sender, EventArgs e)
        {
            loading = true;
            if (!loadedfromfile)
                Loadsettings();
        ...
        private void SitePlanUI_Resize(object sender, EventArgs e)
        {
            map.ZoomAndCenterMarkers("polygons");
        }

        // Load/Save
        public void LoadSitePlan()
        {
            System.Xml.Serialization.XmlSerializer reader = new System.Xml.Serialization.XmlSerializer(typeof(SitePlanData));

            using (OpenFileDialog ofd = new OpenFileDialog())
            {
                ofd.Filter = "*.siteplan|*.siteplan";
                ofd.ShowDialog();
                ...
                if (File.Exists(ofd.FileName))
                {
                    ...
                    {
                        var test = (SitePlanData)reader.Deserialize(sr);

                        loading = true;
                        LoadSitePlanData(test);
                        loading = false;
                    }
                }
            }
        ...
        public void SaveSitePlan()
        {
            System.Xml.Serialization.XmlSerializer writer = new System.Xml.Serialization.XmlSerializer(typeof(SitePlanData));

            var siteplandata = SaveSitePlanData();

            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                sfd.Filter = "*.siteplan|*.siteplan";
                sfd.ShowDialog();
                ...
                if (sfd.FileName != "")
                {
                    ...
                    {
                        writer.Serialize(sw, siteplandata);
                    }
                }
            }
        }

        void LoadSitePlanData(SitePlanData siteplandata)
        {
            list = siteplandata.poly;

            CMB_camera.Text = siteplandata.camera;
            NUM_BenchHeight.Value = siteplandata.benchheight;
            ...
            NUM_split.Value = siteplandata.splitmission;

            NUM_BermDepth.Value = siteplandata.bermdepth;
            NUM_Benches.Value = siteplandata.numbenches;
            NUM_cameraPitch.Value = siteplandata.camerapitch;
            NUM_toeHeight.Value = siteplandata.toeheight;
            CHK_camPitchUnlock.Checked = siteplandata.campitchunlock;

            num_overlap.Value = siteplandata.overlap;
            num_sidelap.Value = siteplandata.sidelap;
            num_picture_per_waypoint.Value = siteplandata.pictures_per_waypoint;
            num_picture_overlap_angle.Value = siteplandata.picture_overlap_angle;
            NUM_maxlength.Value = siteplandata.maxlength;

            rad_trigdist.Checked = siteplandata.trigdist;
            rad_digicam.Checked = siteplandata.digicam;
            rad_repeatservo.Checked = siteplandata.repeatservo;
            chk_stopstart.Checked = siteplandata.breaktrigdist;

            NUM_reptservo.Value = siteplandata.repeatservo_no;
            num_reptpwm.Value = siteplandata.repeatservo_pwm;
            NUM_repttime.Value = siteplandata.repeatservo_cycle;

            num_setservono.Value = siteplandata.setservo_no;
            num_setservolow.Value = siteplandata.setservo_low;
            num_setservohigh.Value = siteplandata.setservo_high;

            CHK_gndcollect.Checked = siteplandata.gndcollect;
            CHK_extraimages.Checked = siteplandata.extraimages;

            NUM_toepoint.Value = siteplandata.height_test;
            NUM_toepoint_runs.Value = siteplandata.toepoint_runs;

            // Copter Settings
            NUM_copter_delay.Value = siteplandata.copter_delay;

            loadedfromfile = true;
        }

        SitePlanData SaveSitePlanData()
        {
            SitePlanData siteplandata = new SitePlanData();

            siteplandata.poly = list;

            siteplandata.camera = CMB_camera.Text;
            siteplandata.benchheight = NUM_BenchHeight.Value;
            siteplandata.angle = NUM_angle.Value;
            ...
            siteplandata.overlap = num_overlap.Value;
            ...
            siteplandata.camerapitch = NUM_cameraPitch.Value;
            siteplandata.toeheight = NUM_toeHeight.Value;
            siteplandata.campitchunlock = CHK_camPitchUnlock.Checked;

            siteplandata.gndcollect = CHK_gndcollect.Checked;
            siteplandata.extraimages = CHK_extraimages.Checked;

            siteplandata.height_test = NUM_toepoint.Value;
            siteplandata.toepoint_runs = NUM_toepoint_runs.Value;

            // Copter Settings
            siteplandata.copter_delay = NUM_copter_delay.Value;

            siteplandata.trigdist = rad_trigdist.Checked;
            siteplandata.digicam = rad_digicam.Checked;
            siteplandata.repeatservo = rad_repeatservo.Checked;
            siteplandata.breaktrigdist = chk_stopstart.Checked;

            siteplandata.repeatservo_no = NUM_reptservo.Value;
            siteplandata.repeatservo_pwm = num_reptpwm.Value;
            siteplandata.repeatservo_cycle = NUM_repttime.Value;

            siteplandata.setservo_no = num_setservono.Value;
            siteplandata.setservo_low = num_setservolow.Value;
            siteplandata.setservo_high = num_setservohigh.Value;
        ...
        void Loadsettings()
        {
            if (plugin.Host.config.ContainsKey("siteplan_camera"))
            {
                LoadSetting("siteplan_benchheight", NUM_BenchHeight);
                LoadSetting("siteplan_facedir", CHK_facedirection);
                LoadSetting("siteplan_autotakeoff", CHK_toandland);
                LoadSetting("siteplan_autotakeoff_RTL", CHK_toandland_RTL);
                LoadSetting("siteplan_followpathhome", CHK_FollowPathHome);
                LoadSetting("siteplan_benchangle", NUM_angle);
                LoadSetting("siteplan_bermdepth", NUM_BermDepth);
                LoadSetting("siteplan_numbenches", NUM_Benches);
                LoadSetting("siteplan_campitch", NUM_cameraPitch);
                LoadSetting("siteplan_toeheight", NUM_toeHeight);
                LoadSetting("siteplan_unlockcampitch", CHK_camPitchUnlock);
                LoadSetting("siteplan_grncollect", CHK_gndcollect);
                LoadSetting("siteplan_extraimages", CHK_extraimages);
                             
                LoadSetting("siteplan_overlap", num_overlap);
                ...
                LoadSetting("siteplan_toepoint_runs", NUM_toepoint_runs);
                             
                LoadSetting("siteplan_trigdist", rad_trigdist);
                LoadSetting("siteplan_digicam", rad_digicam);
                LoadSetting("siteplan_repeatservo", rad_repeatservo);
                LoadSetting("siteplan_breakstopstart", chk_stopstart);
                             
                LoadSetting("siteplan_repeatservo_no", NUM_reptservo);
                LoadSetting("siteplan_repeatservo_pwm", num_reptpwm);
                LoadSetting("siteplan_repeatservo_cycle", NUM_repttime);

                // camera last as it invokes a reload
                LoadSetting("siteplan_camera", CMB_camera);

                // Copter Settings
                LoadSetting("siteplan_copter_delay", NUM_copter_delay);
            }
        }
        ...
        void SaveSetting()
        {
            plugin.Host.config["siteplan_camera"] = CMB_camera.Text;
            plugin.Host.config["siteplan_benchheight"] = NUM_BenchHeight.Value.ToString();
            plugin.Host.config["siteplan_benchangle"] = NUM_angle.Value.ToString();
            plugin.Host.config["siteplan_facedir"] = CHK_facedirection.Checked.ToString();
            plugin.Host.config["siteplan_autotakeoff"] = CHK_toandland.Checked.ToString();
            plugin.Host.config["siteplan_autotakeoff_RTL"] = CHK_toandland_RTL.Checked.ToString();
            plugin.Host.config["siteplan_followpathhome"] = CHK_FollowPathHome.Checked.ToString();
            plugin.Host.config["siteplan_bermdepth"] = NUM_BermDepth.Value.ToString();
            plugin.Host.config["siteplan_numbenches"] = NUM_Benches.Value.ToString();
            plugin.Host.config["siteplan_campitch"] = NUM_cameraPitch.Value.ToString();
            plugin.Host.config["siteplan_toeheight"] = NUM_toeHeight.Value.ToString();
            plugin.Host.config["siteplan_unlockcampitch"] = CHK_camPitchUnlock.Checked.ToString();
            plugin.Host.config["siteplan_gndcollect"] = CHK_gndcollect.Checked.ToString();
            ...
            plugin.Host.config["siteplan_trigdist"] = rad_trigdist.Checked.ToString();
            plugin.Host.config["siteplan_digicam"] = rad_digicam.Checked.ToString();
            plugin.Host.config["siteplan_repeatservo"] = rad_repeatservo.Checked.ToString();
            plugin.Host.config["siteplan_breakstopstart"] = chk_stopstart.Checked.ToString();
                                
            plugin.Host.config["siteplan_repeatservo_no"] = NUM_reptservo.Value.ToString();
            plugin.Host.config["siteplan_repeatservo_pwm"] = num_reptpwm.Value.ToString();
            plugin.Host.config["siteplan_repeatservo_cycle"] = NUM_repttime.Value.ToString();
                                
            // Copter Settings  siteplan
            plugin.Host.config["siteplan_copter_delay"] = NUM_copter_delay.Value.ToString();
        }
        ...
        void DoCalc()
        {
            try
            {
                ...
                double minlength = (double)NUM_maxlength.Value;
        ...
        private void BUT_Accept_Click(object sender, EventArgs e)
        {
            ...
            if (grid != null && grid.Count > 0)
            {
                ...
                var gridobject = SaveSitePlanData();
