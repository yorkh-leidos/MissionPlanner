...
namespace MissionPlanner
{
    public class SitePlan
    {
        ...
        public static List<PointLatLngAlt> CreateCorridor(List<PointLatLngAlt> polygon, double height, double camViewHeight, double camVertSpacing, double distance, double angle,
            double camPitch, bool flipDirection, double bermDepth, int numBenches, double toeHeight, double toepoint, double toepoint_runs, bool pathHome, double homeAlt, double maxLength, FlightPlanner.altmode altmode)
        {
            ...
            for (int bench = 0; bench < numBenches; bench++)
            {
                ...
                for (int lane = 0; lane < lanes; lane++)
                {
                    ...
                    //Seperate segments greater than determined lengths
                    for (int i = 1; i < ans.Count-1; i++)
                    {
                        PointLatLngAlt firstpos = ans[i];
                        PointLatLngAlt nextpos = ans[i + 1];

                        if (firstpos.GetDistance(nextpos) > maxLength)
                        {
                            var newpos = firstpos.newpos(firstpos.GetBearing(nextpos), maxLength);
                            newpos.Tag = "M";
                            ans.Insert(i + 1, newpos);
                        }                        
                    }
